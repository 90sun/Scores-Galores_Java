...
//					tier="A";
				...
				double FTtotalHomeScoredRecProb = (FTneutralHomeScoredRecProb + FThomeScoredRecProb) / 2;
				double FTtotalAwayScoredRecProb = (FTneutralAwayScoredRecProb + FTawayScoredRecProb) / 2;
				...
				double FTtotalHomeConcededRecProb = (FTneutralHomeConcededRecProb + FThomeConcededRecProb) / 2;
				double FTtotalAwayConcededRecProb = (FTneutralAwayConcededRecProb + FTawayConcededRecProb) / 2;
				...
				double FTtotalHomeExpectedRecProb = (FTtotalHomeScoredRecProb + FTtotalAwayConcededRecProb) / 2;
				double FTtotalAwayExpectedRecProb = (FTtotalAwayScoredRecProb + FTtotalHomeConcededRecProb) / 2;
				...
				double HTtotalHomeScoredRecProb = (HTneutralHomeScoredRecProb + HThomeScoredRecProb) / 2;
				double HTtotalAwayScoredRecProb = (HTneutralAwayScoredRecProb + HTawayScoredRecProb) / 2;
				double HTtotalScoredRecProb = HTtotalHomeScoredRecProb + HTtotalAwayScoredRecProb;

				double HTtotalHomeConcededRecProb = (HTneutralHomeConcededRecProb + HThomeConcededRecProb) / 2;
				double HTtotalAwayConcededRecProb = (HTneutralAwayConcededRecProb + HTawayConcededRecProb) / 2;
				double HTtotalConcededRecProb = HTtotalHomeConcededRecProb + HTtotalAwayConcededRecProb;

				double HTtotalHomeExpectedRecProb = (HTtotalHomeScoredRecProb + HTtotalAwayConcededRecProb) / 2;
				double HTtotalAwayExpectedRecProb = (HTtotalAwayScoredRecProb + HTtotalHomeConcededRecProb) / 2;

				double totalHomeScoredAGoalExpectedRecProb = (neutralHomeScoredAGoalExpectedRecProb
						+ homeScoredAGoalExpectedRecProb) / 2;
				double totalAwayScoredAGoalExpectedRecProb = (neutralAwayScoredAGoalExpectedRecProb
						+ awayScoredAGoalExpectedRecProb) / 2;

				double totalTotalExpectedRecProb = Math.max(totalHomeScoredAGoalExpectedRecProb,
						totalAwayScoredAGoalExpectedRecProb);

				double totalHomeFormRecProb = (neutralHomeFormRecProb + homeFormRecProb) / 2;
				double totalAwayFormRecProb = (neutralAwayFormRecProb + awayFormRecProb) / 2;

				double FTtotalHomeOneRecProb = (FTneutralHomeOneRecProb + FThomeOneRecProb) / 2;
				double HTtotalHomeOneRecProb = (HTneutralHomeOneRecProb + HThomeOneRecProb) / 2;

				double FTtotalHomeTwoRecProb = (neutralHomeTwoRecProb + homeTwoRecProb) / 2;
				double FTtotalHomeThreeRecProb = (neutralHomeThreeRecProb + homeThreeRecProb) / 2;
				double FTtotalHomeFourRecProb = (neutralHomeFourRecProb = homeFourRecProb) / 2;
				double FTtotalAwayOneRecProb = (FTneutralAwayOneRecProb + FTawayOneRecProb) / 2;
				double HTtotalAwayOneRecProb = (HTneutralAwayOneRecProb + HTawayOneRecProb) / 2;

				double FTtotalAwayTwoRecProb = (neutralAwayTwoRecProb + awayTwoRecProb) / 2;
				double FTtotalAwayThreeRecProb = (neutralAwayThreeRecProb + awayThreeRecProb) / 2;
				double FTtotalAwayFourRecProb = (neutralAwayFourRecProb = awayFourRecProb) / 2;
				...
				double FTtotalOne = (FTtotalHomeOneRecProb + FTtotalAwayOneRecProb) / 2;
				double HTtotalOne = (HTtotalHomeOneRecProb + HTtotalAwayOneRecProb) / 2;

				double FTtotalTwo = (FTtotalHomeTwoRecProb + FTtotalAwayTwoRecProb) / 2;
				double FTtotalThree = (FTtotalHomeThreeRecProb + FTtotalAwayThreeRecProb) / 2;
				double FTtotalFour = (FTtotalHomeFourRecProb + FTtotalAwayFourRecProb) / 2;
				...
				if (FTtotalMinExpectedRecProb > 0.9999)
					betThis = "0.5";
...
//
				if ((league2.contains("Qatar") && !homeName.contains("Al-Sadd")) || (league2.contains("UAE")
						...
						|| league2.contains("Georgia Division") || league2.contains("Jordan")
						|| league2.contains("Palestine") || league2.contains("Palestine")
						...
						|| (league2.contains("Bolivia") && !homeName.contains("Bolivar")
								&& !homeName.contains("Strongest"))
...
//						|| league2.contains("England U23 1.Liga")
				...
				if (FTtotalMinExpectedRecProb < 2.1) {
					continue;
				...
				if (formOddsDiff < 0.65 && !htBetThis.contentEquals("YES"))
					continue;

				if (formOddsDiff < 0)
					continue;

				if (HTtotalMinOne < 0.6) {
					continue;
				}
			...
			if (totalMaxForm < 1.5 && HTtotalMinOne < 0.8)
				...
				true
					...
					System.out.format("|%-25s|", Double.toString(homeOdds));
					System.out.format("|%-25s|", Double.toString(awayOdds));
